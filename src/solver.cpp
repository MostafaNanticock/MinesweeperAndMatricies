...
list<Move>* solver::getMoves(Board* board, logger* log)
{
    ...
    for(list<Position>::const_iterator it = nonCompletedPositions.begin(); it != nonCompletedPositions.end(); ++it)
    {
        for(int i = 0; i < 8; ++i)
        {
    ...
    std::cout << "Non flagged positions: " << nonCompletedPositions.size() << "\n";
    (*log) << "Total Squares: " << currentSquareId << "\n" << logger::endl;
    ...
    (*log) << "Id: Position\n" << logger::endl;
    for(map<int, int>::const_iterator iter = idToPosition.begin(); iter != idToPosition.end(); ++iter)
    {
    ...
    for(matrix<double>::height_size_type row = firstNonZeroRow; row >= 0 && row <= firstNonZeroRow; --row)
    {
        ...
        for(matrix<double>::width_size_type col = row + 1; col < maxVariableColumn; ++col)
        {
            ...
            if(pivotVal == 0.0 && currentValue != 0.0)
            {
                ...
                std::cout << "Pivot updated to: " << pivot << " => " << currentValue << "\n";
            }
        ...
        (*log) << "value: " << val << " (" << (failedToFindValue ? "true" : "false") << ")" << "\n" << logger::endl;
        ...
        if(pivotVal != 0.0)
        {
            if(failedToFindValue)
            {
                ...
                if(val == minValue)
                {
                    ...
                    for(matrix<double>::width_size_type col = row; col < maxVariableColumn; ++col)
                    {
                        ...
                        if(currentValue > 0.0)
                        {
                            std::cout << "Col " << col << " is not a mine." << "\n";
            ...
            else
            {
                ...
                {
                    std::cout << "Already found pivot for: " << pivot << "\n";
                }
                else
                {
                    std::cout << "Found standard result: " << pivot << " => " << val << "\n";
                    if(val == 0.0 || val == 1.0)
                    {
                    ...
                    {
                        (*log) << "Found pivot value is not 0 or 1 ... what do we do?" << "\n";
                    }
                }
            }
        }
        ...
        {
            std::cout << "There in now pivot in row: " << row << "\n";
        }
    }
